...
class PhysicalMaterialIntegrationTests(unittest.TestCase):
    ...
    def test_wrapper_applies_fidelity_without_rebuilding_shader(self):
        ...
        bsdf = SimpleNamespace(inputs=FakeSockets([
            FakeSocket("Base Color", (0.25, 0.5, 0.75, 1.0)), FakeSocket("Roughness", 0.4),
            FakeSocket("Subsurface Radius", (0.1, 0.1, 0.1)), FakeSocket("Subsurface Scale", 0.1),
            FakeSocket("Emission Strength", 1.0),
        ]))
        output = SimpleNamespace(node=bsdf)
        material = FakeMaterial()
        graph_node = SimpleNamespace(parameters={
            "coating": 1.0, "coat_affect_color": 0.5, "trans_roughness_lock": False,
            "trans_roughness": 0.2, "trans_roughness_inv": True, "trans_depth": 4.0,
            "scattering": 0.5, "sss_color": [0.8, 0.4, 0.2, 1.0],
            "sss_scatter_color": [0.9, 0.2, 0.1, 1.0], "sss_depth": 6.0, "sss_scale": 0.5,
            "emission": 0.5, "emit_luminance": 1200.0, "emit_kelvin": 6500.0,
        })
        calls = []
        ...
        result = integration._apply_physical_fidelity(object(), "tree", graph_node, material, (), 10, 20)
    ...
    def test_wrapper_uses_case_insensitive_parameter_view(self):
        ...
        material = FakeMaterial()
        graph_node = SimpleNamespace(parameters={
            "TRANS_ROUGHNESS_LOCK": False, "TRANS_ROUGHNESS": 0.2,
            "TRANS_ROUGHNESS_INV": True, "TRANS_DEPTH": 4.0,
        })
        def original(self, tree, node, mat, stack, x, y):
            return output
        integration._ORIGINAL = original
    ...
    def test_sss_defaults_are_not_materialized_when_scattering_is_zero(self):
        integration = self._load_integration()
        bsdf = SimpleNamespace(inputs=FakeSockets([
            FakeSocket("Subsurface Radius", (1.0, 0.2, 0.1)), FakeSocket("Subsurface Scale", 0.05),
        ]))
        output = SimpleNamespace(node=bsdf)
        material = FakeMaterial()
    ...
    def test_linked_sockets_are_not_overwritten(self):
        ...
        bsdf = SimpleNamespace(inputs=FakeSockets([
            ...
            FakeSocket("Subsurface Scale", 0.05, True), FakeSocket("Roughness", 0.4, True),
        ]))
        output = SimpleNamespace(node=bsdf)
        material = FakeMaterial()
        graph_node = SimpleNamespace(parameters={
            "scattering": 0.5, "sss_color": [0.8, 0.4, 0.2, 1.0], "sss_depth": 6.0,
            "sss_scale": 0.5, "emission": 0.5, "emit_luminance": 1200.0,
        })
        integration._ORIGINAL = lambda *args: output
        integration._apply_physical_fidelity(object(), "tree", graph_node, material, (), 0, 0)
    ...
    def test_import_path_installs_before_package_and_adapter_work(self):
        ...
        class FakeAdapter:
            def __init__(self, context): events.append(("adapter", context))
            def import_package(self, package, apply_recommended_scale=True):
                events.append(("import", package, apply_recommended_scale))
                return ImportSummary(asset_name="asset", object_count=1, material_count=1, image_count=0, warnings=(), notes=())
        ...
        class FakePackageContext:
            def __enter__(self):
                events.append(("open",)); return SimpleNamespace(manifest=SimpleNamespace(graph=()))
            def __exit__(self, *args): events.append(("close",))
        fake_package_module.open_blendmax = lambda path: FakePackageContext()
        def record_install(): events.append(("install",))
        importer = importlib.import_module("blendmax_blender.importer")
        with patch.dict(sys.modules, {
            "bpy": fake_bpy, "blendmax_blender.blender_adapter": fake_adapter_module,
            "blendmax_blender.package": fake_package_module,
            "blendmax_blender.physical_material_integration": ModuleType("blendmax_blender.physical_material_integration"),
        }):
