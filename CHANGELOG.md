...
- Restart-notice suppression is now scoped to both the current Blender process
  and the exact imported BlendMax version. Installing a newer version in the
  same Blender process can therefore surface a new restart requirement.
...
- Added hot-reload state-payload coverage and a same-process newer-version
  regression test.
